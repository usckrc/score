...
![](`r here::here("Week 4 CellChat", "Images", "Burfeindphysgenpaper.png")`)
...
CACPR_integrated <- readRDS("CACPR_integrated.rds")
...
# Part V: Save the CellChat object and run the "RunCellChat" app
The RunCellChat app is an awesome user interface where a shiny app is pulled up and one can easily visualize all of the significant receptor-ligand pairings.
```{r eval=FALSE}
saveRDS(cellchat, file = "cellchat_Score.rds")
cellchat_score <-readRDS("cellchat_Score.rds")


runCellChatApp(cellchat_score)
```
