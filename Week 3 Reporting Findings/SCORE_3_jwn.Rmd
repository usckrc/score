...
-These are the raw output files directly generated by the sequencing machine (e.g., Illumina NovaSeq, NextSeq, etc.):
-In single-cell/single-nuclei RNA sequencing, you obtain short reads from mRNA that was already reverse transcribed into cDNA.
Main files
![](`r here::here("Week 3 Reporting Findings", "Images", "IMG_003.png")`)


Why are these files compressed (.gz)?
FASTQ files are huge. They’re compressed with gzip to reduce size. A single experiment can easily generate hundreds of GBs.
What happens next?
...
After generating the raw FASTQ files, the next step is to process the data using a pipeline like: Cell Ranger (10X Genomics)
These tools:
-Match reads to individual cells using barcodes
-Deduplicate reads using UMIs
-Align the reads to a genome or transcriptome
-Build the count matrix: genes × cells
![](`r here::here("Week 3 Reporting Findings", "Images", "IMG_003.png")`)

Output folder structure (Cell Ranger)
...
an be used with CellBender, Alevin, or custom workflows
