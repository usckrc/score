...
![](`r here::here("Week 3 Reporting Findings", "Images", "Gantt-chart.webp")`)
...
g.gantt <- gather(df2, "state", "date", 3:4) %>% mutate(date = as.Date(date, "%m/%d/%Y"))

g.gantt2 <- gather(df3, "state", "date", 3:4) %>% mutate(date = as.Date(date, "%m/%d/%Y"))

# Combine the start and end into a single timeline for each Stage
g.gantt_combined <- g.gantt %>%
  ...
  summarise(
    ...
    Mentor = "Institution" # Set as a constant value to show as a single row
  ) %>%
  ungroup()

g.gantt$Stage <- factor(g.gantt$Stage, levels = c("University of Washington", "Oregon Health & Science University", "University of Southern California", "Undergraduate", "Graduate School", "Postdoc", "Junior Faculty"))


# Set the factor levels for the 'Stage' variable
g.gantt_combined$Stage <- factor(g.gantt_combined$Stage,
  levels = c(
    "University of Washington",
    "Oregon Health & Science University",
    "University of Southern California",
...
g.gantt2$Stage <- factor(g.gantt2$Stage,
  levels = c(
    "University of Washington",
    "Oregon Health & Science University",
    "University of Southern California",
...
ggplot() +
  # Plot the segments (timeline for each project) from the first dataframe
  geom_segment(
    data = g.gantt_combined,
    aes(x = start_date, xend = end_date, y = Mentor, yend = Mentor, color = Stage),
    size = 10
  ) +
  # Plot the lines (from the second dataframe)
  geom_line(data = g.gantt2, aes(x = date, y = Mentor, color = Stage), size = 10) +
  labs(x = "Year", y = NULL, title = "Mentors", color = "Institution and Career Stage") +
  theme_minimal(base_size = 15) +
  scale_y_discrete(limits = desired_order) + # Control y-axis order
  theme(
    panel.grid = element_blank(),
    plot.title = element_text(hjust = 0.5, size = 18), # Centered title with larger font
    axis.title = element_text(size = 16),
    legend.title = element_text(size = 16, face = "bold"), # Set legend title size and style
    legend.text = element_text(size = 16),
    axis.text = element_text(size = 14), # Axis labels and ticks
